...
void odom_gps_Callback(const nav_msgs::Odometry::ConstPtr& msg){
    ...
    // car_yaw =  msg->pose.pose.position.z;
}
...
void Compute(){
    ...
    if (bigobs == false){ // Small Obs
        ...
        double max_y_offset = 3.5; // 목표 지점들의 y 범위 (- max_y_offset ~ + max_y_offset)
...
int main(int argc, char** argv){
    ...
    // PUBLISHER
    vizselpath_pub = nh_.advertise<nav_msgs::Path>("viz_SelPath", 10);
    vizallpath_pub = nh_.advertise<visualization_msgs::MarkerArray>("viz_Path81", 10);
    vizCheckArea_pub = nh_.advertise<visualization_msgs::Marker>("viz_CheckArea", 10);
    vizTM_pub = nh_.advertise<visualization_msgs::Marker>("viz_TrackingMarker", 10);
    vizCM_pub = nh_.advertise<visualization_msgs::Marker>("viz_CrashDistMarker", 10);
